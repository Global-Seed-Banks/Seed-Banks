...
# remove NA values 
sb_rich_samps <- sb_prep %>% filter(!is.na(Total_Species),
...
rich_no_samps_fitted <- cbind(rich_no_samps$data,
                          fitted(rich_no_samps, re_formula = NA
                          )) %>%
  as_tibble() %>% inner_join(sb_rich_samps %>% select(Total_Species, 
...
head(rich_no_samps_fitted)
nrow(rich_no_samps_fitted)

# fixed effect coefficients
rich_no_samps_fixef <- fixef(rich_no_samps)
head(rich_no_samps_fixef)

# Random effect coefficients
rich_no_samps_coef <- coef(rich_no_samps)
rich_no_samps_coef # dont really need this



setwd(paste0(path2wd, 'Data/'))
# # # save data objects to avoid doing this every time
save(rich_no_samps_fitted, rich_no_samps_fixef, rich_no_samps_coef, file = 'rich_no_samps.mod_dat.Rdata')


# plots
setwd(paste0(path2wd, 'Data/'))
#load('rich.area.poisson.mod_dat.Rdata')
load('rich_no_samps.mod_dat.Rdata')

# plot richness area biome_broad relationship

head(sb_rich_samps_biome_broad)
...
fig_rich.biome_broad <- ggplot() + 
  ...
  # fixed effect
  geom_line(data = rich_no_samps_fitted,
            aes(#x = (Total_Sample_Area_mm2/1000000), 
              # x = Total_Sample_Area_mm2,
              x = Total_Sample_Area_m2,
  ...
  geom_ribbon(data = rich_no_samps_fitted,
              aes( #x =  (Total_Sample_Area_mm2/1000000), 
                #x =  Total_Sample_Area_mm2, 
                x = Total_Sample_Area_m2,
                ymin = Q2.5, ymax = Q97.5, fill = Biome_Broad_Hab),
              alpha = 0.1 ) +
  #coord_cartesian(xlim = c(min(sb_rich_samps_biome_broad$Total_Sample_Area_m2), quantile(sb_rich_samps_biome_broad$Total_Sample_Area_m2, 0.97))) +
...
rich_no_samps.fixed.p <- as_draws_df(rich_no_samps, subset = floor(runif(n = 1000, 1, max = 2000)))

rich_no_samps.fixed.p
nrow(rich_no_samps.fixed.p)
head(rich_no_samps.fixed.p)
colnames(rich_no_samps.fixed.p)

# select columns of interests and give meaningful names
rich_no_samps_global_posterior <-  rich_no_samps.fixed.p %>% 
  as_tibble() %>%
...
View(rich_no_samps_global_posterior)
head(rich_no_samps_global_posterior)


sb_rich_samps_zone$biome_broad_WWF<- as.factor(sb_rich_samps_zone$biome_broad_WWF)
levels(sb_rich_samps_zone$biome_broad_WWF)
...
# rich.tund.global
rich.aq.p <-  rich_no_samps_global_posterior %>% 
...
rich.arable.p <-  rich_no_samps_global_posterior %>% 
...
rich.bor.p <-  rich_no_samps_global_posterior %>% 
...
rich.des.p <-  rich_no_samps_global_posterior %>% 
...
# rich.fgs.p <-  rich_no_samps_global_posterior %>% 
...
#   dplyr::select(c(eff,eff_upper,eff_lower,response)) %>% distinct() 
# 
# rich.mang.p <-  rich_no_samps_global_posterior %>% 
...
rich.medfs.p <-  rich_no_samps_global_posterior %>% 
...
rich.mongs.p <-  rich_no_samps_global_posterior %>% 
...
rich.tempbf.p <-  rich_no_samps_global_posterior %>% 
...
rich.tempcf.p <-  rich_no_samps_global_posterior %>% 
...
rich.tempgs.p <-  rich_no_samps_global_posterior %>% 
...
# rich.tropcf.p <-  rich_no_samps_global_posterior %>% 
...
#   dplyr::select(c(eff,eff_upper,eff_lower,response)) %>% distinct() 
# 
# rich.tropdbf.p <-  rich_no_samps_global_posterior %>% 
...
#rich.tropf.global
rich.tropf.p <-  rich_no_samps_global_posterior %>% 
...
rich.tropgs.p <-  rich_no_samps_global_posterior %>% 
...
# rich.tropmbf.p <-  rich_no_samps_global_posterior %>% 
...
rich.tund.p <-  rich_no_samps_global_posterior %>% 
...
global.rich_no_samps.p <- bind_rows(rich.aq.p, rich.arable.p, rich.bor.p, rich.des.p,
...
head(global.rich_no_samps.p)

setwd(paste0(path2wd, 'Data/'))
# save data objects to avoid time of compiling every time
save(global.rich_no_samps.p, file = 'global.rich_no_samps.posteriors.Rdata')

 setwd(paste0(path2wd, 'Tables/'))
 write.csv(global.rich_no_samps.p, "table_1.csv")

head(global.rich_no_samps.p)

fig_rich_no_samps_global <- ggplot() + 
  geom_point(data = global.rich_no_samps.p, aes(x = `WWF Biome`, y = Estimate, color=`WWF Biome`),size = 2) +
  geom_errorbar(data = global.rich_no_samps.p, aes(x = `WWF Biome`, ymin = `Lower CI`,
...
# overall legend
rich_no_samps_leg <- g_legend(fig_rich.biome_broad)

# lnadscape 10x16
(fig_rich.biome_broad + theme(legend.position="none") | fig_rich_no_samps_global) / (rich_no_samps_leg) + 
