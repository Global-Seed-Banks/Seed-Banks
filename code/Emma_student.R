...
load( 'gsb_rich_samps-student.Rdata')
...
rich.samps_fitted <- cbind(rich.samps$data,
  ...
  as_tibble() %>% inner_join(sb_prep %>% select(Total_Species, Total_Species2, 
                               ...
                               by= c("Biome_WWF_Zone","Habitat_Broad","studyID", "rowID")
  )
...
save(rich.samps_fitted, rich.samps_fixef, obs_nest.rich.samps, file = 'rich.samps.student.mod_dat.Rdata')
...
fig_rich.samps <- ggplot() + 
  ...
  geom_line(data = obs_nest.rich.samps  %>% unnest(cols = c(data, predicted)) ,
            aes(x = Total_Number_Samples, y= exp(predicted[,1]),
                group = Habitat_Broad,
                colour = Habitat_Broad),
            size = 1.2) +
  # fixed effect
  geom_line(data = rich.samps_fitted,
            aes(x = Total_Number_Samples, y = exp(Estimate)),
            size = 1.5) +
  # uncertainy in fixed effect
  geom_ribbon(data = rich.samps_fitted,
              aes(x = Total_Number_Samples, ymin = exp(Q2.5), ymax = exp(Q97.5)),
              alpha = 0.3) +
